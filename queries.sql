...
-- 2) Count the number of cars for each owner. Display the owners first_name, 
...
SELECT first_name, last_name, ROUND(AVG(vehicles.price)) AS avg_price, COUNT(vehicles.id) AS count
FROM owners
JOIN vehicles ON owners.id = vehicles.owner_id
GROUP BY owners.first_name, owners.last_name
